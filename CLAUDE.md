...
./script/build_and_run.sh --release    # Developer ID-signed, notarized dist/WhisperDesk.dmg (needs local cert + notarytool credentials, see README)
...
**Serial job queue, one GPU consumer at a time.** Jobs (`TranscriptionJob`, `Sources/Models/TranscriptionJob.swift`) move through `JobStatus`/`JobStage` and are processed one at a time by design (`AppModel.runNextQueuedJob`-style logic) — never assume concurrent transcription runs. `JobStore` (`Sources/Stores/JobStore.swift`) persists each job as its own JSON file under `~/Library/Application Support/WhisperDesk/jobs/`, written atomically off the main thread; a job caught "running" at load time (crash/force-quit) is sanitized back to `.canceled` rather than left stuck. A legacy single `jobs.json` is migrated to per-job files on first launch and kept as a backup, never deleted.
...
- `Tests/WhisperDeskTests` — swift-testing suite (`import Testing`, `@Test func ...`, `@testable import WhisperDesk`)
