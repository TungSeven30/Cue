...
struct SubtitleReaderTests {
    ...
    @Test func cliReadNamesTheFileWhenNoCuesAreFound() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("cue-reader-cli-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
        let url = dir.appendingPathComponent("empty.srt")
        try "WEBVTT\n\nnothing here\n".write(to: url, atomically: true, encoding: .utf8)

        do {
            _ = try SubtitleReader.read(contentsOf: url)
            Issue.record("expected read to fail")
        } catch let failure as SubtitleReadFailure {
            #expect(failure.localizedDescription.contains("empty.srt"))
        } catch {
